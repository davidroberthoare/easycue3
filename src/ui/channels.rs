...
fn render_instrument_list(ui: &mut Ui, app: &mut EasyCueApp) {
    ...
    let cols = ((ui.available_width() + GAP) / (TILE_W + GAP))
        .floor()
        .max(1.0) as usize;
    ...
    let scroll_output = egui::ScrollArea::vertical()
        ...
        .show(ui, |ui| {
            ...
            let items: Vec<_> = patches
                .iter()
                .filter_map(|p| {
                    app.fixtures
                        .get_profile(&p.profile_id)
                        .cloned()
                        .map(|prof| (p.clone(), prof))
                })
                .collect();
    ...
    let bg_clicked = ui.input(|i| {
        if let Some(pos) = i.pointer.interact_pos() {
            scroll_output.inner_rect.contains(pos)
...
fn render_fixture_tile(
        ...
        if let Some(universe) = app.universes.first() {
            if profile.has_intensity() {
                ...
                app.virtual_intensity
                    .get_intensity(fixture_id)
                    .unwrap_or_else(|| {
                        app.virtual_intensity
                            .calculate_intensity(fixture_id, universe, patch, profile)
                    })
            } else {
                0.0
            }
            ...
            .map(|u| {
                if profile.has_intensity() {
                    profile
                        .get_parameter_offset(&FixtureParameter::Intensity)
                        .map(|off| {
                            u.get_channel(patch.start_address + off).unwrap_or(0) as f32 / 100.0
                        })
                        .unwrap_or(0.0)
                } else if profile.is_rgb() {
                    app.virtual_intensity
                        .calculate_intensity(fixture_id, u, patch, profile)
                } else {
                    0.0
                }
            })
        ...
        if modifiers.shift {
            if let Some(last_id) = app.ui_state.last_selected_fixture {
                ...
                if let (Some(a), Some(b)) = (a, b) {
                    ...
                    for p in &all[lo..=hi] {
                        app.ui_state.selected_fixtures.insert(p.id);
                    }
                }
            }
    ...
    if display_intensity > 0.0 {
        ...
        let bar_rect = egui::Rect::from_min_max(
            ...
            egui::pos2(
                rect.min.x + 2.0 + (rect.width() - 4.0) * display_intensity,
                rect.max.y - 1.0,
            ),
        );
        let bar_color = if fx_active {
            FX_COLOR
        } else {
            intensity_color(display_intensity)
        };
...
fn render_channel_grid(ui: &mut Ui, app: &mut EasyCueApp) {
    ...
    loop {
        let width_needed =
            (groups_per_row + 1) as f32 * group_width + groups_per_row as f32 * group_spacing; // N+1 groups need N gaps
        if width_needed <= available_width {
            groups_per_row += 1;
            if groups_per_row >= 20 {
...
fn render_channel_box(ui: &mut Ui, app: &mut EasyCueApp, channel: u16) {
    // Get current value
    ...
    if response.dragged() {
        ...
        if total_change != 0 {
            if is_selected && app.ui_state.selected_channels.len() > 1 {
                ...
                let new_master =
                    (app.ui_state.group_master as i32 + total_change).clamp(0, 100) as u8;
                ...
                if max_base > 0 {
                    ...
                    for &ch in &app.ui_state.selected_channels {
                        if let Some(&base_level) = app.ui_state.channel_base_levels.get(&ch) {
                            let output = ((new_master as f32) * (base_level as f32)
                                / (max_base as f32))
                                .round() as u8;
    ...
    let ch_galley = ui.painter().layout_no_wrap(ch_text, ch_font, Color32::GRAY);
    let ch_pos = Pos2::new(
    ...
    let value_galley = ui
        .painter()
        .layout_no_wrap(value_text, value_font, value_color);
